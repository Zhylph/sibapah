...
class Admin extends CI_Controller
{
    ...
    public function pdfberita($id_permintaan)
    {
        $this->load->library('dompdf_gen');
        $data['permintaan'] = $this->Permintaan_model->getIDPermintaan($id_permintaan);
    ...
    public function riwayatmasuk()
    {
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('laporan/l_masuk', $data);
        $this->load->view('templates/footer');
    }

    public function cetakbarangmasuk()
    {
        $data['title'] = 'Riwayat Barang Masuk';
        $data['user'] = $this->db->get_where('tbl_user', ['username' => $this->session->userdata('username')])->row_array();

        $bmasuk = $this->Penerimaan_model;
        $data['penerimaan'] = $bmasuk->getBMasuk();

        $data['tanggal1'] = $this->input->post('tanggal1');
        $data['tanggal2'] = $this->input->post('tanggal2');

        $this->load->library('dompdf_gen');
        $this->load->view('laporan/c_masuk', $data);
    ...
    public function riwayatkeluar()
    {
        ...
        $data['permintaan'] = $this->Permintaan_model->getPermintaanS();
        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('laporan/l_keluar', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function cetakbarangkeluar()
    {
        ...
        $bkeluar = $this->Permintaan_model;
        $data['permintaan'] = $bkeluar->getBKeluar();

        $data['tanggal1'] = $this->input->post('tanggal1');
        $data['tanggal2'] = $this->input->post('tanggal2');

        $this->load->library('dompdf_gen');
        $this->load->view('laporan/c_keluar', $data);
